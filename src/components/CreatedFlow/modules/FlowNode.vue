<template>
  <el-tooltip :disabled="isDisabled()" content="點擊關閉 tooltip 功能">
    ...
    >
      <el-icon
        :class="
          node.icon || node.defaultIcon || 'iconfont icon-gaibanxianxingtubiao-'
        "
      />
      <span class="flex-item">{{ node.name }}</span>
    </div>
    ...
    >
      <div
        ...
        :style="{
          top: 0,
          left: 0,
          cursor: currentTool.type == 'drag' ? 'move' : 'default',
        }"
        @click.stop="selectNode"
        @contextmenu.stop="showNodeContextMenu"
      >
...
export default {
  ...
  data() {
    return {
      ...
      setInfo: {
        NodeRejectType: 0, // 駁回類型
        NodeConfluenceType: "", // 會簽方式
        NodeDesignate: "", // 節點權限方式
        ThirdPartyUrl: "", // 三方回調URL
        NodeDesignateData: {
          // 節點指定操作人
          users: [],
          roles: [],
  ...
  methods: {
    ...
    registerNode() {
      ...
      const currentSelect = {
        ...that.node,
        setInfo: that.node.setInfo || this.setInfo,
      };
    ...
    selectNode() {
      ...
      that.$emit("isMultiple", (flag) => {
        if (!flag) {
          ...
          const f = that.currentSelectGroup.filter(
            (s) => s.id === that.node.id
          );
          if (f.length <= 0) {
    ...
    isDisabled() {
      ...
      if (
        ...
        const tagname = { 1: "通過", 2: "不通過", 3: "駁回" };
        tips += "<p>處理人：" + this.node.setInfo.UserName + "</p>";
        tips += "<p>結果：" + tagname[this.node.setInfo.Taged] + "</p>";
        tips += "<p>處理時間：" + this.node.setInfo.TagedTime + "</p>";
        tips += "<p>備註：" + this.node.setInfo.Description + "</p></div>";

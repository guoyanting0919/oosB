<template>
  <div class="rule-group-box flex-row">
      ...
      <div class="rule-form flex-item">
        <template v-for="(rule, ruleIndex) in ruleForm.Filters">
          <el-form :key="createKey(rule.Key)">
            ...
            <el-form-item
              style="display: inline-block;width:110px;margin-bottom: 0;margin-right: 5px;"
            >
              <el-select size="mini" v-model="rule.Contrast">
                <el-option
                  v-for="item in contrasts"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
            ...
            >
              ...
              ></el-input>
              <span
                style="position: absolute;right: 0;top:6px;bottom:7px;line-height: 28px;display:inline-block;background: #fff;border: 1px solid #DCDFE6;padding: 0 10px;border-radius: 0 4px 4px 0;cursor:pointer;"
                @click="handleSelectUser(ruleForm, ruleIndex)"
              >
            ...
            <el-button
              ...
              size="mini"
              icon="el-icon-delete"
              v-if="ruleForm.Filters.length > 1"
              @click="deleteRule(ruleForm, ruleIndex)"
            ></el-button>
            <el-button
              type="primary"
              size="mini"
              icon="el-icon-plus"
              v-if="ruleIndex === ruleForm.Filters.length - 1"
        ...
        <template v-if="ruleForm.Children && ruleForm.Children.length > 0">
          <rule-item
            :level="index"
            v-for="(child, index) in ruleForm.Children"
    ...
    <el-dialog
      ...
      width="850px"
      :title="loginKey === '{loginUser}' ? '選擇用戶' : '選擇角色'"
...
export default {
  ...
  data() {
    return {
      ...
      keys: [
        {
          description: "{當前登錄用戶的角色}",
          key: "{loginRole}",
          currentName: "當前登錄用戶的角色",
        },
        {
          description: "{當前登錄的用戶}",
          key: "{loginUser}",
        ...
        {
          description: "{當前登錄用戶的部門}",
          key: "{loginOrg}",
          currentName: "當前登錄用戶的部門",
        },
      ],
      selectDialog: false,
      statusOptions: [
    ...
    getUserList() {
      ...
      users.getList(this.userInfo.listQuery).then((response) => {
        ...
        if (this.comonInfo.datas.length > 0) {
          ...
          this.comonInfo.selectTexts =
            this.comonInfo.selectTexts.length > 0 &&
            this.comonInfo.selectTexts.filter(
              (item) => !this.comonInfo.datas.some((x) => x.name === item)
            );
          this.comonInfo.selectIds =
            this.comonInfo.selectIds.length > 0 &&
            this.comonInfo.selectIds.filter(

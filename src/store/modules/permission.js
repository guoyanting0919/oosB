...
const groupRoutes = (data, dispatch) => {
  const parentPath =
    data.item.url.indexOf("?") > -1
      ? data.item.url.split("?")[0]
      : data.item.url;
  var newPath = {
    path: parentPath || "/",
    component:
      data.item && data.item.parentId === null
        ? Layout
  ...
  newPath.path =
    newPath.path.indexOf("http") > -1
      ? `/iframePage/${newPath.meta.code}`
      : newPath.path;
          ...
          const addrouters = routers
            ...
            .children.filter(
              (itemF) =>
                !constantRouterMap
                  .find((item) => item.name === "layout")
                  .children.some((ele) => ele.path === itemF.path)
            );
          newPaths.length > 0 &&
            newPaths.forEach((item) => {
              if (
                item.path.indexOf("/iframePage") <= -1 &&
                item.children.length <= 0
              ) {
                addrouters.filter((router) => router.path === item.path);
                item.children = addrouters.filter(
